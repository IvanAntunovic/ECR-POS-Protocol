...
public class TestRunner
{
    ...
    public int[] runTest(Test test)
    {
        ...
        Message[] messageFromFile = new Message[2];
        messageFromFile = test.getMessages();
        ...
        for (int i = 0; i < messageFromFile.length && !isReceivedMsgFault; i++)
        {
            // Check message direction
            if (messageFromFile[i].getDirection().equals("send"))
            {
               // Send message to SERIAL PORT
               messageProcessor.sendMessage(messageFromFile[i]); 
               isReceiving = false;
            }
            else if (messageFromFile[i].getDirection().equals("receive"))
            {
            ...
            if(isReceiving && !(messageFromFile[i].getContent().equals(decodedMessage)))
            {
